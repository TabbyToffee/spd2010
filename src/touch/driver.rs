...
impl<'a, I2C: I2c, Ti: InterruptInput> SPD2010Touch<'a, I2C, Ti> {
    ...
    fn read_hdp_status(&mut self, status: &mut HDPStatus) -> Result<(), Error<I2C>> {
    ...
    fn read_remain_hdp_data(&mut self, status: &mut HDPStatus) -> Result<(), Error<I2C>> {
    ...
    pub async fn read_touch_data<DELAY: DelayNs>(
        &mut self,
        delay: &mut DELAY,
        touch_data: &mut TouchData,
        ...
        let touch_status = self.read_status_length(delay).await?;

        if touch_status.status_high.tic_in_bios {
            // println!("in bios");
            self.clear_interrupt(delay).await?; // ACK+re-arm+verify
        ...
        if touch_status.status_high.tic_in_cpu {
            ...
            self.clear_interrupt(delay).await?;
            return Ok(false);
        }
        ...
        if touch_status.status_low.pt_exist || touch_status.status_low.gesture {
            // println!("some data");
            self.read_hdp(touch_status, touch_data)?;
            self.clear_interrupt(delay).await?;

            loop {
                self.read_hdp_status(&mut hdp_status)?;
                if hdp_status.status == 0x82 {
                    self.clear_interrupt(delay).await?; // all done
                    break;
        ...
        if touch_status.status_high.cpu_run && touch_status.status_low.aux {
            // println!("aux");
            self.clear_interrupt(delay).await?; // clear & re-arm
        }
    ...
    pub async fn read<DELAY: DelayNs>(
        &mut self,
        delay: &mut DELAY,
        touch_data: &mut TouchData,
    ) -> Result<bool, Error<I2C>> {
        let new_data = self.read_touch_data(delay, touch_data).await?;
        if new_data {
            // self.clear_interrupt_flag();
            Ok(true)
        } else {
...
pub async fn reset<D: DelayNs, P: OutputPin>(
    delay: &mut D,
    reset_pin: &mut P,
) -> Result<(), P::Error> {
    // reset_pin.set_high()?;
